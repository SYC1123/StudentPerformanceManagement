...
public class ChangeStuInfoNetWorkHelper {
    private static final int HANDLER_MSG_TELL_RECV = 1;

    private ISetStuInfo callback;

    private AppCompatActivity mContent;

    public ChangeStuInfoNetWorkHelper(AppCompatActivity appCompatActivity) {
    ...
    public Handler handler = new Handler() {
        ...
        public void handleMessage(@NonNull Message msg) {
            callback.succeed(msg.obj.toString());
        }
    };

    public void startNetThread(final String host, final int port, final String data, ISetStuInfo myCallback) {
