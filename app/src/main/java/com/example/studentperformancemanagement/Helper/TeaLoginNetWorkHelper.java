...
public class TeaLoginNetWorkHelper {
    ...
    private ITeaLogin callback;

    private AppCompatActivity mContent;

    public TeaLoginNetWorkHelper(AppCompatActivity appCompatActivity) {
    ...
    public Handler handler = new Handler() {
        ...
        public void handleMessage(@NonNull Message msg) {
            if (msg.obj.equals("2")) {
                callback.onteaFalied("账号或密码错误！");
            } else if (msg.obj.equals("3")){
                callback.onteaFalied("该用户不存在！");
            }else {
                callback.onteaSucceed(msg.obj);
            }
        }
    };

    public void startNetThread(final String host, final int port, final String data, ITeaLogin myCallback) {
